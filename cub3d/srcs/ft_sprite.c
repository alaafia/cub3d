...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/14 17:43:22 by alaafia           #+#    #+#             */
/*   Updated: 2021/01/14 17:43:40 by alaafia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sprite->spriteheight = abs((int)(mlx->win.height / (sprite->transformy)));
	sprite->drawstarty = -sprite->spriteheight / 2 + mlx->win.height / 2;
	if (sprite->drawstarty < 0)
		sprite->drawstarty = 0;
	sprite->drawendy = sprite->spriteheight / 2 + mlx->win.height / 2;
	if (sprite->drawendy >= mlx->win.height)
		sprite->drawendy = mlx->win.height;
}

void		ft_sprite_width(t_sprtools *sprite, t_mlx *mlx)
{
	sprite->spritewidth = abs((int)(mlx->win.height / (sprite->transformy)));
